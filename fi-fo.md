<h1 style="text-align: center;font-size:40px"> fi-fo笔记 </h1>
...
int init_filein(NovaMedium* video_file_input, MediumConfig_t& vifile_config) {
    ...
    tmpPoolCfg.i_blk_size = 1920 * 1080 * 3;
    tmpPoolCfg.i_blk_cnt = 35;
...
void threadFunction(NovaMedium* video_file_input) {
    ...
    while (1) {
        ...
        auto p_ret1 = video_file_input->bind(video_file_output);
        if (!p_ret1) {
            ERRPRINTF("bind fail");
            return;
        }
        ...
        auto u_ret = video_file_input->unbind(video_file_output);
        if (u_ret != 0) {
            ERRPRINTF("vi unbind fail");
            return;
        }
